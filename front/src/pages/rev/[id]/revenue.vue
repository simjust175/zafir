  ...
  <div class="revenue-page">
    <header class="page-header">
      ...
      <div class="header-main">
        <div class="header-content">
          <div class="header-badge">Revenue Tracking</div>
          <h1 class="page-title">Project Revenue</h1>
          <p class="page-subtitle">Track invoices sent and payments received for this project</p>
        </div>
      </div>
    </header>
      ...
      <div class="metrics-section">
        <div class="metric-card invoiced">
          <div class="metric-header">
            <div class="metric-icon-wrapper">
              ...
              <svg class="metric-icon" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <button class="add-btn" @click="openDialog('invoiced')" title="Add invoice">
          ...
          <div class="metric-bar">
            <div class="metric-bar-fill invoiced" :style="{ width: '100%' }"></div>
          </div>
        </div>
        ...
        <div class="metric-card paid">
          <div class="metric-header">
            ...
            <button class="add-btn" @click="openDialog('paid')" title="Add payment">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <line x1="12" y1="5" x2="12" y2="19" />
                <line x1="5" y1="12" x2="19" y2="12" />
              </svg>
          ...
          <div class="metric-bar">
            <div class="metric-bar-fill paid" :style="{ width: totalInvoiced > 0 ? `${Math.min(percentPaid, 100)}%` : '0%' }"></div>
          </div>
        </div>

        <div class="metric-card outstanding" :class="{ positive: outstanding <= 0 }">
          <div class="metric-header">
            <div class="metric-icon-wrapper outstanding" :class="{ positive: outstanding <= 0 }">
              <div class="metric-icon-bg"></div>
              <svg class="metric-icon" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="metric-body">
            ...
            <span class="metric-value" :class="{ positive: outstanding <= 0 }">{{ formatCurrency(Math.abs(outstanding)) }}</span>
            <span class="metric-meta" :class="{ positive: outstanding <= 0 }">
              {{ outstanding <= 0 ? 'Fully collected' : `${percentPaid}% collected` }}
            </span>
          </div>
          <div class="metric-bar">
            <div class="metric-bar-fill outstanding" :class="{ positive: outstanding <= 0 }" :style="{ width: `${100 - Math.min(percentPaid, 100)}%` }"></div>
          </div>
        </div>
      </div>

      <div class="collection-status-card" v-if="totalInvoiced > 0">
        <div class="status-header">
          <h3>Collection Progress</h3>
          <span class="status-badge" :class="{ complete: percentPaid >= 100 }">
    ...
    <div class="entries-section">
      <div class="entries-column">
        ...
        <div class="entries-list">
          <div v-if="invoicingEntries.length === 0" class="empty-state">
            ...
            <p class="empty-title">No invoices sent yet</p>
            <p class="empty-desc">Record invoices you've sent to your client</p>
            <button class="empty-cta" @click="openDialog('invoiced')">
              Add first invoice
          ...
          <TransitionGroup name="list">
            <div v-for="entry in invoicingEntries" :key="entry.id" class="entry-card">
              <div class="entry-left">
                <div class="entry-indicator invoiced"></div>
              ...
              <div class="entry-actions">
                <button class="action-btn edit" @click="editEntry('invoiced', entry)" title="Edit">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                  </svg>
                </button>
                <button class="action-btn delete" @click="confirmDelete('invoicing', entry)" title="Delete">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      ...
      <div class="entries-column">
        ...
        <div class="entries-list">
          <div v-if="paymentEntries.length === 0" class="empty-state">
            <div class="empty-icon-wrapper">
              <div class="empty-icon-glow success"></div>
            ...
            <p class="empty-title">No payments received yet</p>
            <p class="empty-desc">Record payments received from your client</p>
            <button class="empty-cta" @click="openDialog('paid')">
              Add first payment
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <line x1="5" y1="12" x2="19" y2="12" />
                <polyline points="12 5 19 12 12 19" />
          ...
          <TransitionGroup name="list">
            <div v-for="entry in paymentEntries" :key="entry.id" class="entry-card">
              <div class="entry-left">
                <div class="entry-indicator paid"></div>
                <div class="entry-info">
              ...
              <div class="entry-actions">
                <button class="action-btn edit" @click="editEntry('paid', entry)" title="Edit">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                    <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                  </svg>
                </button>
                <button class="action-btn delete" @click="confirmDelete('payments', entry)" title="Delete">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="3 6 5 6 21 6" />
                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                  </svg>
      ...
      <v-card class="dialog-card">
        <v-card-title class="dialog-title">
          <div class="dialog-icon" :class="dialogType">
            <svg v-if="dialogType === 'invoiced'" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
              <polyline points="14 2 14 8 20 8" />
            </svg>
        ...
        <v-card-actions class="dialog-actions">
          <v-btn variant="text" class="cancel-btn" @click="closeDialog">Cancel</v-btn>
          <v-btn class="save-btn" :class="dialogType" :loading="saving" @click="saveEntry">
    ...
    <v-dialog v-model="showDeleteDialog" max-width="380">
      <v-card class="dialog-card delete-dialog">
        ...
        <v-card-text class="dialog-content">
          <p class="delete-message">Are you sure you want to delete this {{ deleteTarget?.table === 'invoicing' ? 'invoice' : 'payment' }}? This action cannot be undone.</p>
        </v-card-text>
        <v-card-actions class="dialog-actions">
          <v-btn variant="text" class="cancel-btn" @click="showDeleteDialog = false">Cancel</v-btn>
          <v-btn class="delete-btn" :loading="deleting" @click="deleteEntry">Delete</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
