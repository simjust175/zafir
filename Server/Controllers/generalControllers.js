...
class GeneralControllers {
    static async postGeneral({ body, params }, res) {
        ...
        if (!body) return res.status(400).json({ msg: 'Error in GeneralServices/postGeneralService' });
        try {
    ...
    static async getGenerals({ body }, res) {
        if (!body) return res.status(400).json({ msg: 'Error in GeneralServices/getGeneralService' });
        try {
            const Generals = await GeneralService.getService(body);
            if (!Generals) return res.status(404).json({ msg: 'Error in GeneralServices/getGeneralService' });
            res.status(200).json({ Success: `Generals retrieved successfully`, Generals });
        } catch (error) {
            res.status(500).json({ Error: `Error in GeneralServices/getGeneral: ${error.message}` });
        }
    }
    static async geMultipleFiltered(res) {
        ...
        try {
            ...
            if (!generals) return res.status(404).json({ msg: 'Error in GeneralServices/getGeneralService' });
            res.status(200).json({ Success: `Multiple-details retrieved successfully`, generals });
        } catch (error) {
            res.status(500).json({ Error: `Error in GeneralServices/getGeneral: ${error.message}` });
        }
    }
